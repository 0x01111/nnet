...
def plot_curves(tr_err,val_err):
	''' plots the training and validation errors '''
	plt.plot(range(len(tr_err)),tr_err,label='Training error')
	plt.plot(range(len(val_err)),val_err,label='Validation error')
	plt.xlabel('Iterations')
	plt.ylabel('Loss')
	plt.title('Training and Validation set curves')
	plt.legend()
	plt.show()
