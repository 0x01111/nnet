...
class testMultilayerNet(unittest.TestCase):
	...
	def setUp(self):
		...
		for n1,n2 in zip(n_nodes[:-1],n_nodes[1:]):
			self.nnet._weights.append(0.01*np.random.rand(n1+1,n2))
	...
	def test_bprop(self):
		...
		act = self.nnet.fprop(self.X)
		grad = self.nnet.bprop(self.X,self.y,act)
		...
		n = sum(np.size(w) for w in self.nnet._weights)
 		idx = np.random.permutation(n)[:(n/20)] # choose a random 20% 
 		ngrad = [None]*len(idx)

		for i,x in enumerate(idx):
			w_plus = self.nnet.unroll(self.nnet._weights)
			w_minus = self.nnet.unroll(self.nnet._weights)
			...
			act = self.nnet.fprop(self.X,weights_plus)
			loss_plus = self.nnet.compute_loss(act[-1], self.y, weights_plus)
			act = self.nnet.fprop(self.X,weights_minus)
			loss_minus = self.nnet.compute_loss(act[-1], self.y, weights_minus)
