...
class testSparseAutoencoder(unittest.TestCase):
	...
	def test_sae_bprop(self):
		...
		This essentially also tests 'fprop', compute_cost', and a few other fun
		ctions 
		along the way'''
	...
	def test_mln_bprop(self):
		...
		for i,x in enumerate(idx):
			w_plus = utils.unroll(sa.weights)
			w_minus = utils.unroll(sa.weights)
