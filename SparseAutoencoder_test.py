...
class testSparseAutoencoder(unittest.TestCase):
	...
	def setUp(self):
		...
		self.sa = sae.Network() # instance of sparse autoencoder
		self.sa.w_i2h_ = np.random.rand(d+1,self.sa.n_hid)
		self.sa.w_h2o_ = np.random.rand(self.sa.n_hid+1,d)
	...
	def test_bprop(self):
		...
		dE_dw_i2h_,dE_dw_h2o_ = self.sa.bprop(self.X, act, out)
		...
		n = self.sa.w_i2h_.size + self.sa.w_h2o_.size # total number of parameters
 		idx = np.random.permutation(n)[:(n/30)] # choose a random 30% 
 		ngrad = [None]*len(idx) # initialize numerical gradient vector

		for i,x in enumerate(idx):

			w_plus = self.sa.unroll(self.sa.w_i2h_,self.sa.w_h2o_)
			w_minus = self.sa.unroll(self.sa.w_i2h_,self.sa.w_h2o_)
			
			# Perturb one of the weights by eps
			w_plus[x] += eps
			w_minus[x] -= eps
			w_i2h__plus,w_h2o__plus = self.sa.reroll(w_plus)
			w_i2h__minus,w_h2o__minus = self.sa.reroll(w_minus)
