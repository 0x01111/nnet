...
def gradient_descent(wts, update, _X=None, y=None,x_data=None,n_iter=1000,eps=0.35):
	...
	if not _X == None and not y == None:
		for i in range(n_iter):
			...
			wts = [w-eps*g for w,g in zip(wts,grad_wts)]
	...
	for i in range(n_iter):
		...
		wts = [w-eps*g for w,g in zip(wts,grad_wts)] # update the weights

	return wts

def improved_momentum(wts, update, _X=None, y=None,x_data=None,n_iter=1000,eps=0.35,alpha=0.9):
	...
	if not _X == None and not y == None:
		...
		for i in range(n_iter):
			...
			for i,(w,a,g) in enumerate(zip(wts,accum_grad_wts,grad_wts)):
				...
				w -= eps*a # take a step in the new direction
